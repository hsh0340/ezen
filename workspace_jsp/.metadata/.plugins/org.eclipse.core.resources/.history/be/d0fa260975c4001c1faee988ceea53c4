...
< http와 session >
http : 웹에서 사용하는 기본적인 전송 프로토콜
- 특성 : 요청이 완료되면 연결을 끊는다. 연결상태를 유지하지 않음
- stateless 속성
- 일반적인 웹에서의 특성
session :
- 특성 : session이 만료될 때 까지 연결상태를 유지한다.
- state 속성
- ex) 로그인, 은행업무, 웹쇼핑, ...

< cookie와 session >
- 쿠키와 세션은 연결상태를 유지함.
- cookie : 연결정보를 클라이언트 쪽에 저장, 보안이 취약 ex) 웹서핑목록, 웹검색목록 ...
- session : 연결정보를 서버쪽에 저장, 보안이 ex) 로그인 정보 ...
...
6. isNew() : 새로 생성된 세션일 때 true를 리턴
7. invalidate() : 모든 세션을 무효화할 때 사용
8. setAttribute(key, value) : key와 value값으로 세션을 저장할 때 사용
9. getAttribute(key) : 세션의 key로 value값을 얻을 때 사용
10. removeAttribute(key) : 해당 키의 세션을 제거할 때 사용

 --%>
<body>
	<%
	session.setMaxInactiveInterval(10); // 10초 설정
	%>
	<h2>세션 테스트</h2>	
